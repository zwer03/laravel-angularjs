...
class ImportController extends Controller
{
	...
	public function his_posts(Request $request) {
		...
		if ($request->user()->tokenCan('tpi')) {
			...
			try {
				...
				foreach ($transactions as $tx) {
					...
					if($post_visit['status'] == 'A' || $post_visit['status'] == 'F'){
						$patient_name = null;
						...
						$existing_patient_visit = PatientVisit::where([['external_visit_number','=',$post_visit['id']],['patient_id','=',$patient_id]])->first();
						if (!$existing_patient_visit) {
							Log::info('Start process: new patient visit');
							$save_patient_visit = new PatientVisit;
							$save_patient_visit->external_visit_number = $post_visit['id'];
